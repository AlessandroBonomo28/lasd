...
void stestVectorInt(uint& testnum, uint& testerr) {
  ...
  try {
     
    {
      ...
      //vec.Print();
      GetAt(loctestnum, loctesterr, vec, false, 2, 0);// ok
  ...
  
    {
      ...
      //vec.Print();
      //std::cout<<" locstenum"<<loctestnum<<" locsterr "<<loctesterr<<" "<<std::endl; 
      Exists(loctestnum, loctesterr, vec, true, 4); //ERRORE
...
void stestListInt(uint& testnum, uint& testerr) {
  ...
  try {
    ...
    //std::cout<<"back = "<<lst.Back()<<" front = "<<lst.Front()<<std::endl;
    InsertAtFront(loctestnum, loctesterr, lst, true, 9);// segmentation fault!!
   ...
   //err
    ...
    FoldPostOrder(loctestnum, loctesterr, lst, true, &FoldMultiply<int>, 0, 1, 1620);// si rompe
    ...
    //std::cout<<"lst dio brutto"<<std::endl;
    //lst.Print();
    
    lasd::List<int> coplst(lst);
    //std::cout<<"coplist(lst)"<<std::endl;
    //coplst.Print();
